...
async def on_start(self):
    ...
    async def do_mousemove(self, x, y):
        ...
        await self.rdpconn.send_mouse(
            MOUSEBUTTON.MOUSEBUTTON_LEFT, int(x), int(y), False
        )
    ...
    async def do_rightclick(self, x, y):
        ...
        for clicked in [True, False]:
            await self.rdpconn.send_mouse(
                MOUSEBUTTON.MOUSEBUTTON_RIGHT, int(x), int(y), clicked
            )
    ...
    async def do_doubleclick(self, x, y):
        ...
        for clicked in [True, False, True, False]:
            await self.rdpconn.send_mouse(
                MOUSEBUTTON.MOUSEBUTTON_LEFT, int(x), int(y), clicked
            )
    ...
    async def do_screenshot(self):
        ...
        fname = "screenshot_%s.png" % datetime.datetime.utcnow().strftime(
            "%Y_%m_%d_%H%MZ"
        )
        ...
        try:
            ...
            if os.path.exists(scriptfile) is False:
                ...
                scriptfile = os.path.join(
                    basedir, "vchannels", "pscmd", "serverscript.ps1"
                )
        except:
    ...
    async def do_pscmd(self, cmd):
        ...
        try:
            if self.pscmd_channelname not in self.rdpconn.iosettings.vchannels:
                print(
                    "PSCMD channel was either not defined while connecting OR the channel name is not the default."
                )
            ...
            if vchannel.channel_active_evt.is_set() is False:
                print(
                    "Channel is defined, but is not active. Did you execute the client code on the server?"
                )
                return
